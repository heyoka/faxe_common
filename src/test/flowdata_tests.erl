...
prepare_ets() ->
   ets:new(field_paths, [set, public, named_table]).
...
to_num_point_test() ->
   ...
   Expected = #data_point{fields =
      #{<<"meta">> => #{<<"a">> => <<"1">>}, <<"val1">> => 2342.55, <<"val2">> => <<"heyy11">>}},
   ?assertEqual(Expected, flowdata:to_num(P)).

to_num_point_path_test() ->
   P = to_num_point(),
   Expected = #data_point{fields =
   #{<<"meta">> => #{<<"a">> => 1}, <<"val1">> => <<"2342.55">>, <<"val2">> => <<"heyy11">>}},
...
to_num_map_test() ->
   P = to_num_point(),
   Expected =  #{<<"meta">> => #{<<"a">> => <<"1">>}, <<"val1">> => 2342.55, <<"val2">> => <<"heyy11">>},
   ?assertEqual(Expected, flowdata:to_num(P#data_point.fields)).

to_num_map_2_test() ->
   P = to_num_point(),
   Expected =  <<>>,
   ?assertEqual(Expected, flowdata:to_num(P#data_point.id)).
